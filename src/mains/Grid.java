...
public class Grid {
    ...
    public boolean checkValid(Grid grid) { 
        int size = grid.size();
        for(int i=0; i<size; i++) { 
            if (grid.chars.length != size) { // Should not be here grid.chars[i].lenght?
                return false;
            }
        }
        return true;
    ...
    public boolean checkValid(Puzzle puzzle) { 
        ...
        for(int i=0; i<size(); i++){
            for(int j=0; j<size(); j++){
                if(chars[i][j] != puzzle.unfilledChar && chars[i][j]!=puzzle.blankSymbol && !puzzle.validLetter(chars[i][j])) { 
                    return false;
                }
            }
		}
